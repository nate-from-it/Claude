...
3. A **NIST / STIG toggle** — switch from the rule-based NIST guidance
   ...
   whichever OS/device you pick (Linux, Windows, network switches/routers,
   Kubernetes).
...
- **STIG mode**: unlike the NIST guidance above, STIG rules aren't
  ...
  | Network | DISA Cisco IOS **Switch** (L2S) STIG + DISA Cisco IOS **Router** STIG | Bridged — the sources only tag CCI numbers; those are resolved to a NIST 800-53 control via [MITRE Vulcan](https://github.com/mitre/vulcan)'s DISA CCI crosswalk table |
  | Kubernetes | DISA Kubernetes STIG | Bridged, same as above |
  | AWS/Azure/GCP, identity, database | — | Not available. DISA doesn't publish cloud-platform STIGs the way it does OS/network baselines, and the one database STIG source (PostgreSQL 9.x) has no CCI or NIST tagging at all to bridge from. |

  The Cisco router source (`CISCO-IOS-RTR-STIG`) has real rule content only
  on its `devel` branch — `main` is an empty stub, which is why router
  coverage was previously marked unavailable here. Its NDM/firewall/ASA/
  Juniper/Palo Alto/F5 siblings were checked too; none of them exist as
  real `ansible-lockdown` repos yet (only empty or 404 stubs), so they're
  still not ingestible until a tagged source shows up.
...
five `ansible-lockdown` STIG repos plus MITRE Vulcan's CCI crosswalk table.
Note the Cisco router repo needs its `devel` branch — `main` is an empty
stub with no rule content:
...
git clone --depth 1 --branch devel https://github.com/ansible-lockdown/CISCO-IOS-RTR-STIG
...
- STIG mode's NIST crosswalk is Rev 4-based (see above) and covers only
  Linux, Windows, network switches/routers, and Kubernetes — not cloud
  platforms, identity, or databases. Extending it further needs a STIG
